...
func (storage CartRepo) FindCarts(ctx context.Context, userID int) ([]entity.Cart, error)  {
	...
	toppingSql, _, _ := sq.Select("id", "name", "price").From("toppings").Where("id= $1").ToSql()
...
func (storage CartRepo) SaveToCart(ctx context.Context, cart entity.Cart) error {
	...
	Values(cart.User_Id, cart.Product_Id, cart.Price, cart.Qty,  pq.Array(cart.ToppingIds)).Suffix("RETURNING id").ToSql()
