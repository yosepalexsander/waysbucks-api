...
func (s *AddressServer) GetUserAddress(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
...
func (s *AddressServer) CreateUserAddress(w http.ResponseWriter, r *http.Request)  {
	ctx := r.Context()
	claims, ok := ctx.Value(middleware.TokenCtxKey).(*middleware.MyClaims)
